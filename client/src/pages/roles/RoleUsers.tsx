...
export default factory(function RoleUsers({ properties, middleware: { store, icache } }) {
    ...
    const {content: users, first, last, totalElements, number, size, numberOfElements} = pagedUser || defaultPagination;
    ...
    return (
        <div classes={[c.container_fluid]}>
            <div classes={[c.card]}>
                ...
                <h3 classes={[c.card_title]}><strong>{roleName}</strong>: {`${roleUsers.length}`} 个用户</h3>
                    <div classes={["form-check", "form-check-inline", c.ml_3]}>
                        <input type="checkbox" class="form-check-input" id="cbxAssign" onclick={(event: MouseEvent<HTMLInputElement>)=> {
                            const checked = event.target.checked;
                            const view = checked? "edit":"view";
                            icache.set("view", view);
                        }}/>
                        <label class="form-check-label" for="cbxAssign">分配</label>
                    </div>
                ...
                {
                    view === "view" && <table classes={[c.table, c.table_bordered, c.table_hover]}>
                            <thead>
                                <tr>
                                    <th>登录名</th>
                                    <th>用户名</th>
                            ...
                            <tbody>
                                {
                                    roleUsers.length === 0 ? <tr key="empty"><td colspan="5" classes={[c.text_center, c.text_muted]}>没有分配用户！</td></tr> :
                                    roleUsers.map(user => <tr key={user.id}>
                ...
                {
                    view === "edit" && <virtual><table classes={[c.table, c.table_bordered, c.table_hover]}>
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            {
                                users.length === 0 ? <tr key="empty"><td colspan="6" classes={[c.text_center, c.text_muted]}>没有用户！</td></tr> :
                                        ...
                                        <input 
                                            ...
                                            onclick={(event: MouseEvent<HTMLInputElement>) => {
                                                ...
                                                if(checked) {
                                                    ...
                                                    executor(removeUserFromRoleProcess)({roleId, userId: user.id});
                                                }
                                            }
                                        }/>
                                    </td>
