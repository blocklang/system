...
export default factory(function Tree({ properties, middleware: {store, icache} }){
    ...
    const renderTreeNode = (item: ResourceInfo) => {
        ...
        if(hasChildren) { 
            ...
            return (<div key={id} classes={[]}>
                <div key="item" classes={[ c.d_flex, css.node, c.align_items_start]} onclick={(event: MouseEvent<EventTarget>)=>{
                    icache.set<string>("activeNodeId", id);
                    onSelectNode && onSelectNode(id);
                }}>
                    {timesIndent(level)}
                    <div classes={[c.form_check]}><input id={item.id} onclick={(event: MouseEvent<HTMLInputElement>)=>{
                        const checked = event.target.checked;
                        const action = checked?"add":"remove";
                        executor(prepareRoleResourcesProcess)({resourceId: id, action});
                    }} checked={isChecked(id)} classes={[c.form_check_input]} type="checkbox" />
                    <label for={item.id} classes={[c.form_check_label]}>{name}</label>
                    </div>
                    <span classes={[c.flex_grow_1, c.d_flex, "flex-row", c.ml_4]}>
                        {getChildren(id).filter(item=>item.resourceType === "03").map(item => {
                ...
                {
                    (<div key="container" classes={[]}> {getChildren(id).filter(item=>item.resourceType !== "03").map(item => {
                        icache.set(`${item.id}-level`, level+1);
                        return renderTreeNode(item);
                    })}</div>)
                }
        ...
        if(item.resourceType === "03") {
            return <span key={item.id} classes={[css.node, c.mx_2, c.text_muted]} onclick={()=>{
                icache.set<string>("activeNodeId", id);
                onSelectNode && onSelectNode(id);
            }}>
                <div classes={[c.form_check]}>
                    <input id={item.id} onclick={(event: MouseEvent<HTMLInputElement>)=>{
                        const checked = event.target.checked;
                        const action = checked?"add":"remove";
                        executor(prepareRoleResourcesProcess)({resourceId: id, action});
                    }} checked={isChecked(id)} classes={[c.form_check_input]} type="checkbox" />
        ...
        return (<div key={id} classes={[]}>
            <div classes={[c.d_flex, css.node, c.align_items_start]} onclick={()=>{
                icache.set<string>("activeNodeId", id);
                onSelectNode && onSelectNode(id);
            }}>
                {timesIndent(level)}
                <div classes={[c.form_check]}>
                    <input id={item.id} checked={isChecked(id)} classes={[c.form_check_input]} onclick={(event: MouseEvent<HTMLInputElement>)=>{
                        const checked = event.target.checked;
                        const action = checked?"add":"remove";
                        executor(prepareRoleResourcesProcess)({resourceId: id, action});
                    }} type="checkbox" />
    ...
    const renderTree = () => {
        ...
        {
            resources.length === 0 && <div classes={[c.text_muted, c.text_center, c.my_5]}>当前 APP 下没有配置资源！</div>
        }
        </virtual>;
    }
