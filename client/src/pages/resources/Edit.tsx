...
import { setResourceFieldProcess, updateResourceProcess } from "../../processes/resourceProcesses";
...
export default factory(function Edit({ properties, middleware: { store } }) {
	...
	const resource = get(path("resource")) || {};
	const { name = "", url = "", icon = "", resourceType, description = "", auth = "" } = resource;
		...
		<div classes={[c.container_fluid]}>
			<div classes={[c.card]}>
				<div classes={[c.card_header]}>
					<h3 classes={[c.card_title]}>修改资源</h3>
				</div>
				<form role="form" classes={[c.needs_validation]} novalidate={true}>
					<div classes={[c.card_body]}>
						<div classes={[c.form_group]}>
							...
							<div>
								<div classes={[c.form_check, c.form_check_inline]}>
									...
									/>
									<label classes={[c.form_check_label]} for="rdoFunction">
										目录
									</label>
								</div>
								<div classes={[c.form_check, c.form_check_inline]}>
									<input
										checked={resourceType === "02"}
										classes={[c.form_check_input]}
										type="radio"
										name="resourceType"
										id="rdoProgram"
									...
									/>
									<label classes={[c.form_check_label]} for="rdoProgram">
										菜单
									</label>
								</div>
								<div classes={[c.form_check, c.form_check_inline]}>
									<input
										checked={resourceType === "03"}
										classes={[c.form_check_input]}
										type="radio"
										name="resourceType"
										id="rdoOperator"
									...
									/>
									<label classes={[c.form_check_label]} for="rdoOperator">
										按钮
									</label>
								</div>
							</div>
						...
						<div classes={[c.form_group, c.position_relative]}>
							<label for="iptAuth">
								权限标识<small classes={[c.text_muted, c.ml_1]}>必填</small>
							</label>
							<input
								type="text"
								value={auth}
								classes={[c.form_control, showValidationClass("auth")]}
								id="iptAuth"
								oninput={(event: KeyboardEvent<HTMLInputElement>) => {
						...
						<div classes={[c.form_group, c.position_relative]}>
							...
							<input
								...
								id="iptDescription"
								oninput={(event: KeyboardEvent<HTMLInputElement>) => {
									executor(setResourceFieldProcess)({
										field: "description",
					...
					<div classes={[c.card_footer]}>
						...
						<button
							...
							onclick={() => {
								executor(updateResourceProcess)({});
							}}
						>
							保存
						</button>
