...
export default factory(function List({ properties, middleware: {store} }){
    ...
    if(selectedNode && selectedNode.resourceType === "03") {
        return <div key="operator" classes={[c.text_muted, c.text_center, c.mt_5, c.pb_5]}>
            按钮资源下不允许配置资源
        </div>
    }
    ...
    return (
        <div classes={[c.container_fluid]}>
            ...
            <table classes={[c.table, c.table_bordered, c.table_hover]}>
                ...
                <tbody>
                    {
                        children.length === 0 ? <tr key="empty"><td colspan="6" classes={[c.text_center, c.text_muted]}>{`没有配置资源！`}</td></tr> :
                        children.map(item => {
                            return (<tr key={item.id}>
                                ...
                                <td key="icon"><FontAwesomeIcon icon={item.icon as IconName}/> {item.icon}</td>
                                <td key="operators">
