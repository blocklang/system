...
const setAppFieldCommand = commandFactory<{field: keyof AppInfo, value: string}>(({path, payload:{field, value=""}}) => {
    const result = [];
    result.push(replace(path("app", field), value));
    if(field === "name") {
        if(value.trim() === "") {
            result.push(replace(path("formValidation", field), {status: ValidateStatus.INVALID, message: "请输入APP名称！"}));
        } else {
            // 校验通过，则清空错误信息
...
const saveAppCommand = commandFactory(async({get, path})=>{
    ...
    const {name = ""} = appInfo;
    if(name.trim() === "") {
        return [replace(path("formValidation", "name"), {status: ValidateStatus.INVALID, message: "请输入APP名称！"})];
    }
