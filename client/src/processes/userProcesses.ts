...
const setUserFieldCommand = commandFactory<{field: keyof UserInfo, value: string}>(({path, payload:{field, value=""}}) => {
    const result = [];
    result.push(replace(path("user", field), value));
    if(field === "username") {
        if(value.trim() === "") {
            result.push(replace(path("formValidation", field), {status: ValidateStatus.INVALID, message: "请输入登录名！"}));
        } else {
            // 校验通过，则清空错误信息
	...
	if(field === "password") {
		if(value.trim() === "") {
            result.push(replace(path("formValidation", field), {status: ValidateStatus.INVALID, message: "请输入密码！"}));
        } else {
            // 校验通过，则清空错误信息
            result.push(replace(path("formValidation", field), {status: ValidateStatus.VALID, message: ""}));
        }
...
const saveUserCommand = commandFactory(async({get, path})=>{
	...
	const result = [];
    if(username.trim() === "") {
		result.push(replace(path("formValidation", "username"), {status: ValidateStatus.INVALID, message: "请输入登录名！"}));
	}
	if(password.trim() === "") {
		result.push(replace(path("formValidation", "password"), {status: ValidateStatus.INVALID, message: "请输入密码！"}));
	}
	if(result.length > 0) {
		return result;
	}
