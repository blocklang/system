...
const addUserToRoleCommand = commandFactory<{ roleId: string, userId: string }>(async ({at, get, path, payload: {roleId, userId}}) => {
    const token = get(path("session", "token"));
...
const removeUserFromRoleCommand = commandFactory<{ roleId: string, userId: string }>(async ({at, get, path, payload: {roleId, userId}}) => {
    const token = get(path("session", "token"));
    const users = get(path("users"));
